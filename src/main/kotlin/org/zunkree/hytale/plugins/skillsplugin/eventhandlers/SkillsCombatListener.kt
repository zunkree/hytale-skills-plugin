...
class SkillsCombatListener {
    ...
    fun onPlayerDealDamage(ctx: HexweaveDamageContext) {
        ...
        ctx.logger.debug { "Damage source is player: ${player.displayName}" }

        val skillType = player.inventory.itemInHand
            ?.let {
                ctx.logger.debug { "Player ${player.displayName} is holding item: $it" }
                categoryToSkillType(it.item.id) ?: run {
                    ctx.logger.debug { "No skill type mapping found for item categories: ${it.item.id}" }
                    return
                }
            }
            ?: SkillsType.UNARMED
        ...
        ctx.logger.debug { "Granting $skillType XP: $baseXp (damage: ${ctx.damage.initialAmount}, multiplier: ${config.combatDamageMultiplier})" }
    ...
    private fun categoryToSkillType(itemId: String): SkillsType? {
        logger.debug { "Mapping item '$itemId' to skill type based on categories" }
        for ((prefix, skillType) in WEAPON_PREFIX_TO_SKILL_TYPE) {
            if (itemId.startsWith(prefix)) {
                logger.debug { "Item '$itemId' matched prefix '$prefix', mapped to skill type $skillType" }
                return skillType
            }
        }
        return null
